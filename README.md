 ...
 * `private String reportMessage` - the message that will be reported when this implementation catches a lint error. This `String` can be set at runtime or ignored and overwritten with `report(T t)`
 ...
 * `report(T t)` - function to return `reportMessage` or be overwritten and return a more static string
...
## LintRule
`LintRule` is the class we use to setup what triggers a failure for a lint rule as well as what will happen when we have a failure.
...
``` Java
 ...
 Our `LintRegister` acts as a simple intermediate between non I/O parts of the Lint stack and our I/O parts of our Lint stack, the `LintRunner`
...
In this example we are ensuring a `JSONObject` does not satisfy **all** of:
1. Has a `type` field with a value of `boolean`
